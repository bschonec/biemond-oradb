...
Puppet::Functions.create_function(:'oradb::opatch_version') do
  # determine the opatch version of an oracle home
  ...
  def opatch_version(oracle_home_path)
    ...
    log "lookup fact oradb_inst_opatch#{oracle_home_path}"
    scope = closure_scope
    opatch = scope['facts']["oradb_inst_opatch#{oracle_home_path}"]
    if (opatch == 'NotFound') || opatch.nil?
